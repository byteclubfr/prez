...
function build (from, to, options, notify) {
  ...
  if (!options.skipReveal) {
    ...
    copy(highlightStyles, path.join(to, "css"), notify);
  }
...
function getHighlightThemeLink (theme) {
  return "<link rel=\"stylesheet\" href=\"css/styles/" + theme + ".css\" id=\"highlight-theme\">";
}
...
function generateSubCover (slides) {
  var id = slides[0].id.split("-")[0];
  return "<section id=\"" + id + "\"><h1>" + id + "</h1></section>";
}
