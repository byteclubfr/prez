...
var revealFiles = ["css", "js", "lib", "plugin"].map(x => path.join(pathToReveal, x));
...
function build (from, to, options, notify) {
  ...
  if (!options.skipReveal) {
    revealFiles.forEach((src) => copy(src, to, notify));
  ...
  if (!options.skipUser) {
    userFiles.forEach((f) => copy(path.join(from, f), to, notify));
  }
...
function buildIndex (from, to, options, notify, addAssets) {
  ...
  var parsed = parse(options.parser, slides, slidesDir);
...
function getAssets (from, base) {
  ...
  return (base || []).concat(_.invoke(paths, "trim").filter(_.identity));
}
